...
def count_unique_detail_urls(page: Page) -> int:
    """Menghitung jumlah URL detail lowongan unik yang sudah tampil."""
    hrefs = page.locator(
...
def get_display_progress(page_text: str) -> tuple[int | None, int | None]:
    """
    Membaca teks seperti:
    'Menampilkan 18 dari 24 lowongan'
    """
    match = re.search(
        r"Menampilkan\s+(\d+)\s+dari\s+(\d+)\s+lowongan",
        page_text,
        flags=re.IGNORECASE,
    )
...
def click_load_more(page: Page) -> bool:
    """
    Mencari pemuat halaman berikutnya pada berbagai jenis elemen:
    button, link, elemen role=button, dan tombol ikon pagination.
    """
    text_patterns = [
    ...
    candidate_locators = [
        page.get_by_role("button", name=combined_pattern),
        page.get_by_role("link", name=combined_pattern),
        page.locator("button").filter(has_text=combined_pattern),
        page.locator("a").filter(has_text=combined_pattern),
        page.locator('[role="button"]').filter(has_text=combined_pattern),
        page.locator("text=" + combined_pattern.pattern),
    ]
        ...
        for index in range(count):
            ...
            try:
                ...
                try:
                    ...
                    # Fallback ketika ada overlay atau elemen bukan button asli.
    ...
    # Fallback untuk tombol pagination berupa ikon tanpa teks.
    icon_candidates = [
        page.locator(
    ...
    for locator in icon_candidates:
        try:
            count = locator.count()
        except Exception:
            continue

        # Prioritaskan elemen paling bawah/terakhir.
...
def collect_listing_cards(page: Page) -> list[CardData]:
    ...
    initial_text = clean_text(page.locator("body").inner_text()) or ""
    total_expected = get_total_listings(initial_text)

    displayed, progress_total = get_display_progress(initial_text)
    ...
    if displayed is not None:
        print(f"Data yang tampil pada halaman awal: {displayed}")

    previous_count = count_unique_detail_urls(page)
    no_change_attempts = 0

    for attempt in range(1, MAX_LOAD_MORE_ATTEMPTS + 1):
        current_count = count_unique_detail_urls(page)

        print(
            f"Memuat data: {current_count}"
            + (
                f"/{total_expected}"
                if total_expected is not None
                else ""
        ...
        if total_expected is not None and current_count >= total_expected:
            break

        # Scroll bertahap hingga benar-benar ke bagian bawah.
        ...
        if clicked:
            print("  Tombol halaman berikutnya ditemukan dan ditekan.")

            # Tunggu sampai jumlah link berubah.
            try:
                    ...
                    previous => {
                        ...
                        const urls = new Set(
                            links
                                ...
                                .filter(url =>
                                    /\/magang-nasional\/lowongan\/[^/?#]+\/?$/.test(url)
                                )
                        );

                        return urls.size > previous;
                    }
                    """,
                    arg=current_count,
                    timeout=15_000,
            ...
            # Sebagian situs memuat data hanya dengan infinite scroll.
            page.wait_for_timeout(2_500)

        new_count = count_unique_detail_urls(page)
        ...
        # Coba beberapa kali sebelum menyerah karena render bisa lambat.
        if no_change_attempts >= 3:
            print(
                "Tidak ada tambahan data setelah 3 percobaan. "
                "Proses pemuatan dihentikan."
            )
            break
    ...
    if total_expected and len(cards) < total_expected:
        print(
            f"PERINGATAN: website menyatakan ada {total_expected} lowongan, "
            f"tetapi script baru menemukan {len(cards)}."
        )
    elif total_expected:
        print(
            f"Semua {total_expected} lowongan terfilter berhasil ditemukan."
        )
